  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ...
        ref={ref}
        className={cn(
          "h-full w-px bg-gray-700 absolute left-1.5 rtl:right-1.5 py-3 rounded-md hover:bg-gray-600 duration-300 ease-in-out",
          className,
        )}
        {...props}
      />
    ...
    return (
      <AccordionPrimitive.Item {...props} value={value} className="relative overflow-hidden h-full ">
        <AccordionPrimitive.Trigger
          className={cn(`flex items-center gap-1 text-sm rounded-md text-gray-300`, className, {
            "bg-gray-800 rounded-md": isSelect && isSelectable, // Selected background
        ...
        >
          {expendedItems?.includes(value)
            ? (openIcon ?? <FolderOpenIcon className="h-4 w-4 text-gray-400" />)
            : (closeIcon ?? <FolderIcon className="h-4 w-4 text-gray-400" />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
  ...
  return (
    <AccordionPrimitive.Item value={value} className="relative">
      <AccordionPrimitive.Trigger
        ...
        aria-label="File"
        className={cn(
          "flex items-center gap-1 cursor-pointer text-sm pr-1 rtl:pl-1 rtl:pr-0 rounded-md  duration-200 ease-in-out text-gray-300",
          {
            "bg-gray-800": isSelected && isSelectable, // Selected background
          },
      ...
      >
        {fileIcon ?? <FileIcon className="h-4 w-4 text-gray-400" />}
        {children}
      </AccordionPrimitive.Trigger>
    </AccordionPrimitive.Item>
  )
